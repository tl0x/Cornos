...
public class Container extends FocusableComponent {
	 ...
	 *
	 * @param title
	 *            the caption of the container
	 * @param description
	 *            the description for this component
	 * @param renderer
	 *            the renderer used by the container
	 */
	...
	/**
	 * Render the container. Components are rendered in a column based on the height
	 * they specify via {@link Context#setHeight(int)}. The horizontal border is
	 * defined by {@link Renderer#getBorder()}. The vertical space between to
	 * components is defined by {@link Renderer#getOffset()}.
	 */
	@Override
	public void render(Context context) {
		...
		doComponentLoop(context, (subContext, component) -> {
			...
			if (subContext.isHovered() && subContext.getDescription() != null)
				tempDescription = subContext.getDescription();
		});
	...
	/**
	 * Returns the total height of the container, accounting for the height of its
	 * child components.
	 */
	@Override
	 ...
	 *            the current context
	 * @param posy
	 *            the vertical position of the child component
	...
	protected interface LoopFunction {
		 ...
		 *            the component
		 */
		void loop(Context context, Component component);
	}
}
