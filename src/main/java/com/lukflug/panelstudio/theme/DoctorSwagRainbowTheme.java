...
public class DoctorSwagRainbowTheme implements Theme {
    ...
    public DoctorSwagRainbowTheme(ColorScheme scheme, int height, Supplier<Boolean> ignoreDisabled,
                                  Supplier<Boolean> buttonRainbow) {
        this.scheme = scheme;
    ...
    protected class ComponentRenderer extends RendererBase {
        ...
        public void renderTitle(Context context, String text, boolean focus, boolean active, boolean open) {
            ...
            if (level != 0) {
                ...
                Point p1, p2, p3;
                if (open) {
                    p3 = new Point(context.getPos().x + context.getSize().width - 3,
                            context.getPos().y + context.getSize().height / 4);
                    p2 = new Point(context.getPos().x + context.getSize().width - context.getSize().height / 2,
                            context.getPos().y + context.getSize().height * 3 / 4);
                    p1 = new Point(context.getPos().x + context.getSize().width - context.getSize().height + 3,
                            context.getPos().y + context.getSize().height / 4);
                } else {
                    p3 = new Point(context.getPos().x + context.getSize().width - context.getSize().height * 3 / 4,
                            context.getPos().y + 3);
                    p2 = new Point(context.getPos().x + context.getSize().width - context.getSize().height / 4,
                            context.getPos().y + context.getSize().height / 2);
                    p1 = new Point(context.getPos().x + context.getSize().width - context.getSize().height * 3 / 4,
                            context.getPos().y + context.getSize().height - 3);
                }
            ...
            if (level == 0 && open) {
                ...
                context.getInterface().fillRect(new Rectangle(context.getRect().x,
                                context.getRect().y + context.getRect().height - 1, context.getRect().width, 1), color, color,
                        color, color);
            }
        }
        ...
        public void renderRect(Context context, String text, boolean focus, boolean active, Rectangle rectangle,
            ...
            if ((level == 0 || !active) && (level != 1 || !ignoreDisabled.get())) {
                ...
                context.getInterface().fillRect(
                        new Rectangle(rectangle.x, rectangle.y,
                                context.getRect().x + context.getRect().width - rectangle.x, rectangle.height),
            ...
            if (level == 0)
                stringPos = new Point(rectangle.x + rectangle.width / 2 - context.getInterface().getFontWidth(text) / 2,
                        rectangle.y + getOffset());
            if (level == 2 && overlay)
        ...
        public Color getMainColor(boolean focus, boolean active) {
            if (active)
                return getColorScheme().getActiveColor();
        ...
        protected void renderRainbowRect(Rectangle rect, Context context, boolean focus) {
            ...
            if (getColorScheme().getOpacity() != 0)
                targetHue += rect.height / (float) getColorScheme().getOpacity();
            else
            ...
            while (currentHue < targetHue) {
                ...
                int top = Math.round((currentHue - hsb[0]) * getColorScheme().getOpacity());
                int bottom = Math.round((nextHue - hsb[0]) * getColorScheme().getOpacity());
                context.getInterface().fillRect(new Rectangle(rect.x, rect.y + top, rect.width, bottom - top), colorA,
