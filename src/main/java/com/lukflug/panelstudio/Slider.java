...
public abstract class Slider extends FocusableComponent {
	...
	public void render(Context context) {
		...
		if (attached) {
			double value = (context.getInterface().getMouse().x - context.getPos().x)
					/ (double) (context.getSize().width - 1);
			if (value < 0)
				value = 0;
			else if (value > 1)
				value = 1;
		...
		renderer.renderRect(context, "", hasFocus(context), false, new Rectangle(
				new Point(context.getPos().x + (int) (context.getSize().width * getValue()), context.getPos().y),
				new Dimension((int) (context.getSize().width * (1 - getValue())), renderer.getHeight(false))), false);
		renderer.renderRect(context, title, hasFocus(context), true, new Rectangle(context.getPos(),
				new Dimension((int) (context.getSize().width * getValue()), renderer.getHeight(false))), true);
	}
	 ...
	 *
	 * @param value
	 *            the slider value between 0 (empty) and 1 (full)
	 */
