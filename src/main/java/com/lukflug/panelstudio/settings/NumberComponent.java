...
public class NumberComponent extends Slider {
	 ...
	 * @param renderer
	 *            {@link Renderer} for the component
	 * @param setting
	 *            the setting in question
	 * @param min
	 ...
	 */
	public NumberComponent(String text, String description, Renderer renderer, NumberSetting setting, double min,
			double max) {
	...
	/**
	 * Render the component, with the caption being composed of the setting name and
	 * current value.
	 */
	@Override
	public void render(Context context) {
		if (setting.getPrecision() == 0)
			title = String.format("%s: \u00A77%d", text, (int) setting.getNumber());
		else
			title = String.format("%s: \u00A77%." + setting.getPrecision() + "f", text, setting.getNumber());
	...
	protected double getValue() {
		return (setting.getNumber() - setting.getMinimumValue())
				/ (setting.getMaximumValue() - setting.getMinimumValue());
	}
