...
public class ColorComponent extends CollapsibleContainer {
	...
	/**
	 * Custom {@link ColorScheme} that set the active color to the value of the
	 * setting.
	 */
	 ...
	 * @param renderer
	 *            the renderer for the color setting container
	 * @param animation
	 *            the animation for opening and closing
	 * @param componentRenderer
	 *            the renderer for the children of the container
	 * @param setting
	 *            the setting in question
	 * @param alpha
	 *            whether to render an alpha slider
	 * @param rainbow
	 *            whether to render a rainbow slider
	 * @param colorModel
	 *            {@link Toggleable} indicating whether to use RGB (false) or HSB
	 *            (true)
	 */
	public ColorComponent(String title, String description, Renderer renderer, Animation animation,
			Renderer componentRenderer, ColorSetting setting, boolean alpha, boolean rainbow, Toggleable colorModel) {
		...
		if (rainbow)
			addComponent(new ColorButton(componentRenderer));
		...
		if (alpha)
			addComponent(new ColorSlider(componentRenderer, 3));
	}
	...
	protected class ColorButton extends FocusableComponent {
		 ...
		 *
		 * @param renderer
		 *            the {@link Renderer} for the component
		 */
	...
	protected class ColorSlider extends Slider {
		...
		/**
		 * Override the {@link ColorScheme} and render the component with the caption
		 * containing the name of the component in the color model and the value for
		 * that component.
		 */
		@Override
		public void render(Context context) {
		...
		protected double getValue() {
			...
			if (value < 3) {
				if (colorModel.isOn())
					return Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), null)[value];
				switch (value) {
					case 0 :
			...
			switch (this.value) {
				case 0 :
					if (colorModel.isOn())
						...
						c = new Color((int) (255 * value), c.getGreen(), c.getBlue());
					if (alpha)
						setting.setValue(
								new Color(c.getRed(), c.getGreen(), c.getBlue(), setting.getColor().getAlpha()));
					else
						setting.setValue(c);
					break;
				case 1 :
					if (colorModel.isOn())
						...
						c = new Color(c.getRed(), (int) (255 * value), c.getBlue());
					if (alpha)
						setting.setValue(
								new Color(c.getRed(), c.getGreen(), c.getBlue(), setting.getColor().getAlpha()));
					else
				...
				case 2 :
					if (colorModel.isOn())
						...
						c = new Color(c.getRed(), c.getGreen(), (int) (255 * value));
					if (alpha)
						setting.setValue(
								new Color(c.getRed(), c.getGreen(), c.getBlue(), setting.getColor().getAlpha()));
					else
	...
	/**
	 * {@link ColorScheme} to override the active color to the current value of the
	 * color setting.
	 *
	 * @author lukflug
	 */
	protected class ColorSettingScheme implements ColorScheme {
		...
		/**
		 * Return the current color setting, instead of the active color defined by the
		 * scheme.
		 */
		@Override
