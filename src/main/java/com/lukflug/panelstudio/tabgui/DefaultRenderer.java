...
public class DefaultRenderer implements TabGUIRenderer {
    ...
    public DefaultRenderer(ColorScheme scheme, int height, int border, int up, int down, int left, int right,
                           int enter) {
        this.scheme = scheme;
        this.border = border;
        this.height = height;
    ...
    public void renderCaption(Context context, String caption, int index, int height, boolean active) {
        Color color;
        if (active)
            color = scheme.getActiveColor();
        else
            color = scheme.getFontColor();
        Point p = context.getPos();
