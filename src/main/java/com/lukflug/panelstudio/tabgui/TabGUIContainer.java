...
public class TabGUIContainer implements TabGUIComponent {
	 ...
	 * @param renderer
	 *            the {@link TabGUIRenderer} for this container
	 * @param animation
	 *            the animation for {@link #selectedAnimation}, may be null
	 */
	...
	public void render(Context context) {
		...
		if (selectedAnimation != null)
			offset = (int) (selectedAnimation.getValue() * renderer.getHeight());
	...
	public void handleKey(Context context, int scancode) {
		...
		if (renderer.isEscapeKey(scancode)) {
			...
			if (renderer.isUpKey(scancode)) {
				...
				if (selectedAnimation != null)
					selectedAnimation.setValue(selected);
				...
				if (components.get(selected).select())
					childOpen = true;
			}
		} else {
