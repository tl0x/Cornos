...
public abstract class GLInterface implements Interface {
	 ...
	 *
	 * @param clipX
	 *            whether to clip in the horizontal direction
	 */
	...
	/**
	 * Set OpenGL to the state used by the rendering methods. Should be called
	 * before rendering.
	 */
	...
	/**
	 * Restore OpenGL to the state expected by Minecraft. Should be called after
	 * rendering.
	 */
	...
	public void fillTriangle(Point pos1, Point pos2, Point pos3, Color c1, Color c2, Color c3) {
		...
		bufferbuilder.vertex(pos1.x, pos1.y, getZLevel())
				...
				.next();
		bufferbuilder.vertex(pos2.x, pos2.y, getZLevel())
				.color(c2.getRed() / 255.0f, c2.getGreen() / 255.0f, c2.getBlue() / 255.0f, c2.getAlpha() / 255.0f)
				.next();
		bufferbuilder.vertex(pos3.x, pos3.y, getZLevel())
				.color(c3.getRed() / 255.0f, c3.getGreen() / 255.0f, c3.getBlue() / 255.0f, c3.getAlpha() / 255.0f)
				.next();
		tessellator.draw();
	}
	...
	public void drawLine(Point a, Point b, Color c1, Color c2) {
		...
		bufferbuilder.vertex(a.x, a.y, getZLevel())
				.color(c1.getRed() / 255.0f, c1.getGreen() / 255.0f, c1.getBlue() / 255.0f, c1.getAlpha() / 255.0f)
				.next();
		bufferbuilder.vertex(b.x, b.y, getZLevel())
				.color(c2.getRed() / 255.0f, c2.getGreen() / 255.0f, c2.getBlue() / 255.0f, c2.getAlpha() / 255.0f)
				.next();
		tessellator.draw();
	}
	...
	public void fillRect(Rectangle r, Color c1, Color c2, Color c3, Color c4) {
		...
		bufferbuilder.vertex(r.x, r.y + r.height, getZLevel())
				...
				.next();
		bufferbuilder.vertex(r.x + r.width, r.y + r.height, getZLevel())
				.color(c3.getRed() / 255.0f, c3.getGreen() / 255.0f, c3.getBlue() / 255.0f, c3.getAlpha() / 255.0f)
				.next();
		bufferbuilder.vertex(r.x + r.width, r.y, getZLevel())
				.color(c2.getRed() / 255.0f, c2.getGreen() / 255.0f, c2.getBlue() / 255.0f, c2.getAlpha() / 255.0f)
				.next();
		bufferbuilder.vertex(r.x, r.y, getZLevel())
				.color(c1.getRed() / 255.0f, c1.getGreen() / 255.0f, c1.getBlue() / 255.0f, c1.getAlpha() / 255.0f)
				.next();
		tessellator.draw();
	}
	 ...
	 *
	 * @param matrix
	 *            matrix to multiply with {@link #COORDS}
	 */
	...
	private void project(float x, float y, float z, float w) {
		...
		for (int i = 0; i < 4; i++)
			COORDS[i] = (COORDS[i] / COORDS[3] + 1) / 2;
		COORDS[0] = COORDS[0] * VIEWPORT[2] + VIEWPORT[0];
		COORDS[1] = COORDS[1] * VIEWPORT[3] + VIEWPORT[1];
	}

	/**
	 * Utility function to set clipping rectangle by projecting the coordinates
	 * using gluProject.
	 *
	 * @param r
	...
	protected void scissor(Rectangle r) {
		...
		GL11.glScissor(Math.round(Math.min(x1, x2)), Math.round(Math.min(y1, y2)), Math.round(Math.abs(x2 - x1)),
