...
/**
 * Container that can be closed and scrolled, so that its children can be
 * hidden.
 *
 * @author lukflug
 */
public class CollapsibleContainer extends FocusableComponent implements Toggleable {
	 ...
	 * @param renderer
	 *            the {@link Renderer} for the container
	 * @param open
	 *            the {@link Toggleable} for {@link #open}
	 * @param animation
	 *            the animation for this container
	 * @param toggle
	 *            the {@link Toggleable} to be toggled by the user
	 */
	public CollapsibleContainer(String title, String description, Renderer renderer, Toggleable open,
			Animation animation, Toggleable toggle) {
	...
	/**
	 * Renders a background, title bar, border and, if the container is open, the
	...
	public void render(Context context) {
		...
		if (open.getValue() != 0) {
			...
			scrollPosition = renderer.renderScrollBar(context, hasFocus(context), isActive(), scroll, childHeight,
					scrollPosition);
			if (scrollPosition > childHeight - containerHeight)
				scrollPosition = childHeight - containerHeight;
			if (scrollPosition < 0)
				scrollPosition = 0;
		}
	...
	public void handleButton(Context context, int button) {
		...
		if (context.isClicked() && button == Interface.LBUTTON) {
			if (toggle != null)
				toggle.toggle();
		} else if (context.isHovered() && button == Interface.RBUTTON
				&& context.getInterface().getButton(Interface.RBUTTON)) {
			open.toggle();
		}
		if (open.getValue() == 1) {
	...
	/**
	 * Returns the current height, accounting for whether the container is open or
	 * closed.
	 */
	@Override
	...
	protected int getContainerOffset() {
		...
		if (scrollPosition < 0)
			scrollPosition = 0;
		return (int) (renderer.getHeight(open.getValue() != 0) - scrollPosition
				- (1 - open.getValue()) * containerHeight);
	}
	 ...
	 *
	 * @param childHeight
	 *            the total height of the children
	...
	protected int getRenderHeight(int childHeight) {
		...
		if (scrollPosition < 0)
			scrollPosition = 0;
		return (int) (containerHeight * open.getValue() + renderer.getHeight(open.getValue() != 0)
				+ renderer.getBottomBorder());
	}
	...
	protected Rectangle getClipRect(Context context, int height) {
		return new Rectangle(context.getPos().x + renderer.getLeftBorder(scroll),
				context.getPos().y + renderer.getHeight(open.getValue() != 0),
				context.getSize().width - renderer.getLeftBorder(scroll) - renderer.getRightBorder(scroll),
				getRenderHeight(height) - renderer.getHeight(open.getValue() != 0) - renderer.getBottomBorder());
	}
	 ...
	 *            the current context
	 * @param onTop
	 *            whether the context should be on top
	 * @return the context for the container
	 */
	protected Context getSubContext(Context context, boolean onTop) {
		return new Context(context, renderer.getLeftBorder(scroll), renderer.getRightBorder(scroll),
				getContainerOffset(), hasFocus(context), onTop);
	}
}
