...
public class HUDPanel extends DraggableContainer {
	 ...
	 *            the component
	 * @param renderer
	 *            the renderer for this container
	 * @param open
	 *            toggleable indicating whether the container is open or closed
	 * @param animation
	 *            the animation for opening and closing this container
	 * @param guiOpen
	 *            whether to accept input and render container itself or not
	 * @param minBorder
	 *            the minimum border for the container
	 */
	public HUDPanel(FixedComponent component, Renderer renderer, Toggleable open, Animation animation,
			Toggleable guiOpen, int minBorder) {
		super(component.getTitle(), null, new HUDRenderer(renderer, guiOpen, minBorder), open, animation, null,
				new Point(0, 0), 0);
	...
	public void setPosition(Interface inter, Point position) {
		component.setPosition(inter,
				new Point(position.x, position.y + renderer.getHeight(open.getValue() != 0) + renderer.getOffset()));
	}
	...
	public int getWidth(Interface inter) {
		return component.getWidth(inter) + renderer.getBorder() * 2 + renderer.getLeftBorder(scroll)
				+ renderer.getRightBorder(scroll);
	}
	...
	protected static class HUDRenderer extends RendererProxy {
		...
		/**
		 * Returns the offset defined by the base renderer, if it is larger than
		 * {@link #minBorder}. Otherwise it will return {@link #minBorder}.
		 */
		@Override
		...
		/**
		 * Returns the border defined by the base renderer, if it is larger than
		 * {@link #minBorder}. Otherwise it will return {@link #minBorder}.
		 */
		@Override
		...
		public void renderRect(Context context, String text, boolean focus, boolean active, Rectangle rectangle,
				boolean overlay) {
			if (guiOpen.isOn())
				renderer.renderRect(context, text, focus, active, rectangle, overlay);
		}

		@Override
		public void renderBackground(Context context, boolean focus) {
			if (guiOpen.isOn())
				renderer.renderBackground(context, focus);
		}

		@Override
		public void renderBorder(Context context, boolean focus, boolean active, boolean open) {
			if (guiOpen.isOn())
				renderer.renderBorder(context, focus, active, open);
		}
		...
		public int renderScrollBar(Context context, boolean focus, boolean active, boolean scroll, int childHeight,
				int scrollPosition) {
			if (guiOpen.isOn())
				return renderer.renderScrollBar(context, focus, active, scroll, childHeight, scrollPosition);
			else
		...
		public Color getMainColor(boolean focus, boolean active) {
			if (guiOpen.isOn())
				return renderer.getMainColor(focus, active);
			else
		...
		public Color getBackgroundColor(boolean focus) {
			if (guiOpen.isOn())
				return renderer.getBackgroundColor(focus);
			else
				return new Color(0, 0, 0, 0);
		}
		...
		public Color getFontColor(boolean focus) {
			if (guiOpen.isOn())
				return renderer.getFontColor(focus);
			else
				return new Color(0, 0, 0, 0);
		}
