package me.constantindev.ccl.gui.clickgui;
...
public class ClickGUI extends Screen {
            ...
            if (module.type == newType) {
                ...
                CustomButtonWidget w = new CustomButtonWidget(xOffset, yOffset, 100, 20, Text.of(module.name), () -> {
                    ...
                    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
                        ...
                        } else this.unselectedColor = new Color(86, 113, 113, 50);
    ...
    void handleModuleChange(Module m) {
        ...
        int xOffset = 184;
        int yOffset = 1;
        CustomButtonWidget enabled = new CustomButtonWidget(xOffset, yOffset, 80, 20, Text.of("Enabled"), () -> m.setEnabled(!m.isEnabled())) {
            @Override
            public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
                if (m.isEnabled()) this.unselectedColor = new Color(86, 113, 113, 50);
                else this.unselectedColor = new Color(25, 44, 49, 50);
                super.renderButton(matrices, mouseX, mouseY, delta);
            }
        };
        ...
        for (MConf.ConfigKey configKey : m.mconf.config) {
            if (configKey instanceof MConfToggleable) {
                ...
                CustomButtonWidget btn = new CustomButtonWidget(xOffset, yOffset, 80, 20, Text.of(configKey.key), ((MConfToggleable) configKey)::toggle) {
                    @Override
                    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
