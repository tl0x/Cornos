...
public class TriggerBot extends Module {
	...
	public void onExecute() {
		if (Cornos.minecraft.crosshairTarget instanceof EntityHitResult) {
			...
			if (!ehr.getEntity().isAttackable() || !ehr.getEntity().isAlive())
				return;
			assert Cornos.minecraft.interactionManager != null;
			if (ehr.getEntity() instanceof PlayerEntity
					&& Cornos.friendsManager.getFriends().containsKey(ehr.getEntity().getName().asString())) {
				return;
			}
