...
public class BetterHotbar extends Module {
	...
	public static void renderHotbar() {
		...
		double i = 182;
		Renderer.renderLineScreen((double) w.getScaledWidth() / 2 - (i / 2), w.getScaledHeight() - 22,
				(double) w.getScaledWidth() / 2 + (i / 2), w.getScaledHeight() - 22, Hud.themeColor.getColor(), 2);
		Renderer.renderLineScreen((double) w.getScaledWidth() / 2 - (i / 2), w.getScaledHeight() - 22,
				(double) w.getScaledWidth() / 2 - (i / 2), w.getScaledHeight(), Hud.themeColor.getColor(), 2);
		Renderer.renderLineScreen((double) w.getScaledWidth() / 2 + (i / 2), w.getScaledHeight() - 22,
				(double) w.getScaledWidth() / 2 + (i / 2), w.getScaledHeight(), Hud.themeColor.getColor(), 2);
	}
}
