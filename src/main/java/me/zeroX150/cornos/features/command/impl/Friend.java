        ...
        switch (args[0].toLowerCase()) {
            case "add":
                try {
                    ...
                    Cornos.friendsManager.getFriends().put(name,
                            new me.zeroX150.cornos.etc.config.Friend(name, fakeName));
                    STL.notifyUser("Added " + name + " as a friend!");
                } catch (ArrayIndexOutOfBoundsException e) {
                    STL.notifyUser("Usage: " + Cornos.config.mconf.getByName("prefix").value
                            + "friend add <Player> <FakeName>");
                }
                break;
            case "remove":
                try {
                    String name = args[1];
                    for (Map.Entry<String, me.zeroX150.cornos.etc.config.Friend> entry : Cornos.friendsManager
                            .getFriends().entrySet()) {
                        if (entry.getValue().getRealName().equalsIgnoreCase(name)
                                || entry.getValue().getFakeName().equalsIgnoreCase(name)) {
            ...
            case "rename":
                try {
                    ...
                    STL.notifyUser("Usage: " + Cornos.config.mconf.getByName("prefix").value
                            + "friend rename <Player/FakeName> <NewName>");
                }
                break;
            default:
                STL.notifyUser("Subcommands: add, remove, list, rename");
