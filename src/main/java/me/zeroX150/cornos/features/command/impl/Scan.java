...
import me.zeroX150.cornos.etc.PScanRunner;
import me.zeroX150.cornos.etc.PortScannerManager;
...
public class Scan extends Command {
    ...
    public void onExecute(String[] args) {
        ...
        if (args.length < 5) {
            STL.notifyUser(
                    "I'd appreciate you giving me an ip, port min, port max, thread amount and delay between scans");
        ...
        double maxTime = (Double.parseDouble(args[2]) - Double.parseDouble(args[1])) * 200
        ...
        PScanRunner pscR = new PScanRunner(addr, Integer.parseInt(args[3]), Integer.parseInt(args[4]), 200,
                Integer.parseInt(args[1]), Integer.parseInt(args[2]), scanResults -> {
            STL.notifyUser("Done!");
            int ports = 0;
