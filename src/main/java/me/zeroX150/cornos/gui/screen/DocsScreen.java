...
public class DocsScreen extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (isCommands) {
            ...
            offset = 2 + 3 + theight + 3 + 1;
            for (Command command : CommandRegistry.getAll()) {
                Renderer.renderLineScreen(new Vec3d(2, offset + theight + 1, 0),
                        new Vec3d(2 + 3 + maxWidth + 3 + 4 + maxWidthDesc + 3, offset + theight + 1, 0),
                        Hud.themeColor.getColor().darker().darker(), 2);
                offset += theight + 3;
            }
        }
        ...
        Renderer.renderLineScreen(new Vec3d(2 + 3 + xOffset, 2, 0), new Vec3d(4 + xOffset, h, 0),
                Hud.themeColor.getColor(), 2);
        textRenderer.draw(matrices, desc, 2 + 3 + xOffset + 4, 2 + 3, 0xFFFFFF);
        Renderer.renderLineScreen(new Vec3d(2, 2 + 3 + theight + 1, 0), new Vec3d(w, 2 + 3 + theight + 1, 0),
                Hud.themeColor.getColor(), 2);
